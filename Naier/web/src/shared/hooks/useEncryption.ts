...
export function useEncryption() {
  const identityRef = useRef<KeyBundle | null>(null);

  const createAndStoreKeyPair = useCallback(async () => {
    const nextKeyBundle = await generateKeyBundle();
    identityRef.current = nextKeyBundle;
    await keyStore.saveKeyBundle(nextKeyBundle);
    return nextKeyBundle;
  }, []);
  ...
  const loadKeyPair = useCallback(async () => {
    ...
    const stored = await keyStore.getKeyBundle();
  ...
  const deriveDMChannelKey = useCallback(async (theirPublicKey: string) => {
    ...
    return deriveSharedKey(keyPair.identity.exchange.privateKey, theirPublicKey);
  }, [loadKeyPair]);
  ...
  const signLoginChallenge = useCallback(async (challenge: string) => {
    ...
    return signChallenge(challenge, keyPair.device.signing.privateKey);
  }, [loadKeyPair]);
